"""
Test for SuccessCtr TimeSheets app models.
"""
...
class PayPeriodTests(IntegrationTestCase):
    ...
    def setUp(self):
        try:
            ...
            self.test_pay_period = models.PayPeriod.objects.create(date_start=self.date_start)
    ...
    def test_get_payperiod(self):
        """
        Tests for get_payperiod() static function.
        """
        today = timezone.localdate()

        with self.subTest('No args provided'):
            # Verify it acts as if we passed today's date.

            curr_pay_period = models.PayPeriod.get_payperiod()

            # Verify start date is same as or before today.
            self.assertTrue(curr_pay_period.date_start <= today)
            # Verify end date is same as or after today.
            self.assertTrue(curr_pay_period.date_end >= today)

        with self.subTest('Two weeks ago date provided'):
            two_weeks_ago = today - timezone.timedelta(days=14)
            last_pay_period = models.PayPeriod.get_payperiod(two_weeks_ago)

            # Verify start date is same as or before two_weeks_ago.
            self.assertTrue(last_pay_period.date_start <= two_weeks_ago)
            # Verify end date is same as or after two_weeks_ago.
            self.assertTrue(last_pay_period.date_end >= two_weeks_ago)

            # Verify PayPeriods refer to different models.
            self.assertNotEqual(curr_pay_period, last_pay_period)

        with self.subTest('Four weeks ago datetime provided (timezone.datetime)'):
            four_weeks_ago = timezone.datetime.now() - timezone.timedelta(days=28)
            second_last_pay_period = models.PayPeriod.get_payperiod(four_weeks_ago)
            four_weeks_ago = four_weeks_ago.date()

            # Verify start date is same as or before two_weeks_ago.
            self.assertTrue(second_last_pay_period.date_start <= four_weeks_ago)
            # Verify end date is same as or after two_weeks_ago.
            self.assertTrue(second_last_pay_period.date_end >= four_weeks_ago)

            # Verify PayPeriods refer to different models.
            self.assertNotEqual(second_last_pay_period, last_pay_period)
            self.assertNotEqual(second_last_pay_period, curr_pay_period)

        with self.subTest('Four weeks ago datetime provided (datetime.datetime)'):
...
class TimesheetShiftTests(IntegrationTestCase):
    """
    Tests to ensure valid TimeSheet Shift model creation/logic.
    """
    @classmethod
    def setUpTestData(cls):
    ...
    def setUp(self):
        ...
        self.test_employee_shift = models.TimesheetShift.objects.create(
    ...
    def test_plural_representation(self):
        self.assertEqual(str(self.test_employee_shift._meta.verbose_name), 'Timesheet Shift')
        self.assertEqual(str(self.test_employee_shift._meta.verbose_name_plural), 'Timesheet Shifts')

    def test_dummy_creation(self):
        # Test create.
        dummy_model_1 = models.TimesheetShift.create_dummy_model()
        self.assertIsNotNone(dummy_model_1)
        self.assertTrue(isinstance(dummy_model_1, models.TimesheetShift))

        # Test get.
        dummy_model_2 = models.TimesheetShift.create_dummy_model()
        self.assertIsNotNone(dummy_model_2)
        self.assertTrue(isinstance(dummy_model_2, models.TimesheetShift))
    ...
    def test_clock_in_equal_to_clock_out(self):
        ...
        with self.assertRaises(ValidationError):
            with transaction.atomic():
                models.TimesheetShift.objects.create(employee=self.user_1, clock_in=clock_in, clock_out=self.clock_out)
    ...
    def test_overlapping_shifts_clockin_between_clockout_same(self):
        ...
        shift = models.TimesheetShift.objects.create(
